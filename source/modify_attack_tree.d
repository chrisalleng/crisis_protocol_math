        ...
        ref ModifyTreeNode[] nodes, int current_node)
{
    ...
    if (!fork.required())
    {
        ...
        nodes[current_node].expected_damage = nodes[current_node].after.attack_dice.count(
                DieResult.Hit) + nodes[current_node].after.attack_dice.count(
                DieResult.Wild) + nodes[current_node].after.attack_dice.count(DieResult.Crit);
        return;
    }
    ...
    fork_attack_state(nodes[current_node].after, fork,
